    ...
    (() => {
        ...
        function initSliders() {
            ...
            if (document.querySelector(".presentation__slider")) {
                let swiperSpeed = 600;
                new swiper_core_Swiper(".presentation__slider", {
        ...
        const dynamicHeader = function(header, options = {}) {
            ...
            let headerHeight = headerHeightValue && headerScroll ? headerHeightValue : headerElem.offsetHeight;
        ...
        Array.from(document.querySelectorAll(".js-accordion")).forEach((el => {
            ...
            accordion.toggleItem(0, false, {
                noTransition: false
            });
        }));
        const initHeaders = function() {
            if (document.querySelector(".header__dynamic")) {
                dynamicHeader(".header__dynamic", {
                    ...
                    scrollLock: {
                        scrollLockClass: "scroll-locked",
                        scrollLockDesktop: true,
                        scrollLockArray: [ ".header__dynamic" ]
                    },
            ...
            if (document.querySelector(".hero-iframe__header")) {
                ...
                let dhReload = dynamicHeader(".hero-iframe__header", options);
                const adminPanel = function() {
                    const inputControls = document.querySelectorAll(".input-control");
                    const originalOptions = {
                        ...options
                    };
                    const savedOptions = sessionStorage.getItem("options");
                    if (savedOptions) options = JSON.parse(savedOptions);
                    const bindSliderToOptions = (sliderId, valueId, optionName) => {
                        ...
                        const savedValue = sessionStorage.getItem(optionName);
                        ...
                        slider.addEventListener("change", (() => {
                            ...
                            dhReload.destroy();
                            dhReload = dynamicHeader(".hero-iframe__header", options);
                            sessionStorage.setItem("options", JSON.stringify(options));
                            sessionStorage.setItem(optionName, newValue);
                        }));
                    };
                    const applyOptionsAndInitSliders = () => {
                        dhReload.destroy();
                        dhReload = dynamicHeader(".hero-iframe__header", options);
                        bindSliderToOptions("scroll-margin-slider", "scroll-margin-value", "scrollMargin");
                        bindSliderToOptions("main-scroll-margin-slider", "main-scroll-margin-value", "mainElementScrollMargin");
                        document.getElementById("scroll-margin-slider").value = options.scrollMargin;
                        document.getElementById("scroll-margin-value").textContent = options.scrollMargin;
                        document.getElementById("main-scroll-margin-slider").value = options.mainElementScrollMargin;
                        document.getElementById("main-scroll-margin-value").textContent = options.mainElementScrollMargin;
                    };
                    inputControls.forEach((control => {
                        ...
                        const checkHandler = function() {
                            if (control.checked) {
                                options = {
                                    ...options,
                                    [param]: control.checked
                                };
                                dhReload.destroy();
                            } else options = {
                                ...options,
                                [param]: originalOptions[param]
                            };
                            sessionStorage.setItem("options", JSON.stringify(options));
                            applyOptionsAndInitSliders();
                            if (!control.checked) window.location.reload();
                        };
                        if (control.dataset.type === "checkbox") control.checked = options[param];
                        control.addEventListener("change", checkHandler);
                        if (control.checked) checkHandler();
                    }));
                    const savedScrollOptions = sessionStorage.getItem("options");
                    if (savedScrollOptions) {
                        const parsedOptions = JSON.parse(savedScrollOptions);
                        options.scrollMargin = parsedOptions.scrollMargin;
                        options.mainElementScrollMargin = parsedOptions.mainElementScrollMargin;
                    } else sessionStorage.setItem("options", JSON.stringify(options));
                    applyOptionsAndInitSliders();
                    const mql = window.matchMedia(`(max-width: ${options.mediaQuery}px)`);
                    mql.addEventListener("change", (function() {
                        if (mql.matches) window.location.reload();
                    }));
                };
                adminPanel();
            }
            if (document.querySelector(".secondary-header__pc--1")) {
                const options = {
                    dynamic: true,
            ...
            if (document.querySelector(".secondary-header__mobile--1")) new dynamicHeader(".secondary-header__mobile--1", {
                dynamic: true,
                mainElement: ".dh-docs__main--1",
                mediaQuery: 830,
                scrollMargin: 30,
            ...
            if (document.querySelector(".secondary-header__pc--2")) dynamicHeader(".secondary-header__pc--2", {
                dynamic: false,
        ...
        function setupCheckboxEventHandlers() {
            const containers = document.querySelectorAll(".presentation__slide");
            if (!containers) {
                console.error("Containers not found:");
                return;
            }
            containers.forEach((container => {
                const checkbox = container.querySelector(".change-menu__checkbox");
                const frames = container.querySelectorAll(".presentation__slide-iframe");
                if (!checkbox) {
                    console.error("Checkbox not found in frame:", container);
                    return;
                }
                checkbox.addEventListener("click", (() => {
                    frames.forEach((frame => {
                        frame.classList.toggle("iframe-hidden");
                    }));
                    const desktopFrame = frames[0].contentDocument || frames[0].contentWindow.document;
                    const mobileFrame = frames[1].contentDocument || frames[1].contentWindow.document;
                    const desktopDocument = desktopFrame.querySelector(".dh-docs__page");
                    const mobileDocument = mobileFrame.querySelector(".dh-docs__page");
                    if (frames[0].classList.contains("iframe-hidden")) mobileDocument.scrollTop = desktopDocument.scrollTop; else if (frames[1].classList.contains("iframe-hidden")) desktopDocument.scrollTop = mobileDocument.scrollTop;
                }));
            }));
        }
        ...
        const searchDocs = function() {
            ...
            const contentItems = document.querySelectorAll(".docs__option");
            const searchSuggestionsData = Array.from(contentItems).map((item => item.getAttribute("data-content").split(", ")));
            const recentSuggestions = [];
            let recentSuggestionsContainer;
            const defaultSuggestions = [ "dynamic", "menu", "scrollLock" ];
            const maxSuggestionsToShow = 15;
            let selectedSuggestion = null;
            const wrapMatchingLetters = (text, searchText) => {
                const regex = new RegExp(searchText, "gi");
                return text.replace(regex, (match => `<span class="highlight">${match}</span>`));
            };
            const showDefaultSuggestions = () => {
                searchSuggestionsList.innerHTML = "";
                defaultSuggestions.slice(0, maxSuggestionsToShow).forEach((suggestion => {
                    const suggestionItem = document.createElement("li");
                    const suggestionLink = document.createElement("a");
                    suggestionLink.className = "search-link";
                    suggestionLink.href = "javascript:void(0);";
                    suggestionLink.innerHTML = wrapMatchingLetters(suggestion, searchInput.value);
                ...
                if (recentSuggestions.length > 0) {
                    recentSuggestionsContainer = document.createElement("ul");
                    recentSuggestionsContainer.className = "header__search-history";
                    recentSuggestions.forEach((suggestion => {
                        const recentSuggestionItem = document.createElement("li");
                        const recentSuggestionLink = document.createElement("a");
                        recentSuggestionLink.className = "search-link history-link";
                        recentSuggestionLink.href = "javascript:void(0);";
                        recentSuggestionLink.textContent = suggestion;
                        recentSuggestionLink.addEventListener("click", (() => {
                    ...
                    searchSuggestionsList.appendChild(recentSuggestionsContainer);
                }
                searchSuggestionsList.style.display = "flex";
            };
            const showSuggestions = (suggestions, searchText) => {
                searchSuggestionsList.innerHTML = "";
                suggestions.slice(0, maxSuggestionsToShow).forEach((suggestion => {
                    const suggestionItem = document.createElement("li");
                    const suggestionLink = document.createElement("a");
                    suggestionLink.className = "search-link";
                    suggestionLink.href = "javascript:void(0);";
            ...
            const transliterate = text => text.toLowerCase().split("").map((char => transliterationMap[char] || char)).join("");
            const filterSuggestions = searchText => {
                const filteredSuggestions = [];
                const transliteratedText = transliterate(searchText);
                for (const data of searchSuggestionsData) for (const word of data) if (word.toLowerCase().startsWith(searchText.toLowerCase()) || word.toLowerCase().startsWith(transliteratedText.toLowerCase())) {
                    filteredSuggestions.push(data[0]);
                    break;
                }
            ...
            const updateSuggestions = () => {
                ...
                if (searchText === "") showDefaultSuggestions(); else {
                    const filteredSuggestions = filterSuggestions(searchText);
                    showSuggestions(filteredSuggestions, searchText);
                    if (recentSuggestionsContainer) {
                        recentSuggestionsContainer.innerHTML = "";
                        recentSuggestions.forEach((suggestion => {
                            const recentSuggestionItem = document.createElement("li");
                            recentSuggestionItem.className = "search-link recent-suggestion";
                            recentSuggestionItem.textContent = suggestion;
                            recentSuggestionItem.addEventListener("click", (() => {
                                selectedSuggestion = suggestion;
                                navigateToItem(selectedSuggestion);
                                searchInput.value = selectedSuggestion;
                                searchSuggestionsList.style.display = "none";
            ...
            document.addEventListener("click", (event => {
                if (!event.target.matches("#searchInput, #searchSuggestions li")) searchSuggestionsList.style.display = "none";
            }));
            ...
            let scrollTimeout;
            const highlightItem = targetItem => {
                targetItem.classList.add("highlighted");
                setTimeout((() => {
                    targetItem.classList.remove("highlighted");
                }), 1e3);
            };
            const navigateToItem = itemText => {
                let targetItem;
                for (const item of contentItems) {
                    const dataContent = item.getAttribute("data-content").split(", ");
                    for (const word of dataContent) if (word.toLowerCase() === itemText.toLowerCase()) {
                        targetItem = item;
                        break;
                    }
                    if (targetItem) break;
                }
                if (!targetItem) for (const item of contentItems) {
                    const dataContent = item.getAttribute("data-content").split(", ");
                    const mainWord = dataContent[0].toLowerCase();
                    if (mainWord.startsWith(itemText.toLowerCase())) {
                        targetItem = item;
                        break;
                    }
                }
                if (targetItem) {
                    ...
                    const existingIndex = recentSuggestions.indexOf(itemText);
                    if (existingIndex !== -1) recentSuggestions.splice(existingIndex, 1);
                    recentSuggestions.unshift(itemText);
                    if (recentSuggestions.length > 3) recentSuggestions.pop();
                    updateSuggestions();
                }
            };
        };
        const btnUpIntegrate = function() {
            const myBtnUp = document.querySelector(".btn-up");
            const btnUpRightCalculate = function() {
            ...
            btnUpRightCalculate();
            window.addEventListener("resize", btnUpRightCalculate);
        };
        window.addEventListener("load", (function() {
            ...
            new masonry(".docs__content", {
                itemSelector: ".docs__option",
                columnWidth: ".docs__option-size",
                gutter: 20,
